...
public class CoinTweenController : MonoBehaviour
{
    ...
    public void TweenCoins(Vector2 collectibleScreenPoint)
    {
        for(int i = 0; i < maxCoinCount; i++)
        {
            ...
            coinTweenInstance.GetComponent<RectTransform>().DOMove(coinTargetPoint.position,i*speedMultiplier).SetEase(tweenType).OnComplete(()=>GamePlayManager.instance.IncreaseCoinCount());
        }
    }
}
